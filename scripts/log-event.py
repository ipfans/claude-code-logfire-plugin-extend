...
# ---------------------------------------------------------------------------
# Daemon fork — survive Claude Code's aggressive hook killing
# ---------------------------------------------------------------------------


def _daemonize(func: object, *args: object) -> None:
    """Double-fork into a detached daemon that runs *func(*args)*.

    The original (hook) process returns immediately so Claude Code sees a
    clean exit.  The grandchild runs in a new session, immune to SIGHUP /
    SIGTERM sent to the hook's process group.
    """
    first_pid = os.fork()
    if first_pid > 0:
        # Parent — wait for the first child to avoid zombies, then return.
        os.waitpid(first_pid, 0)
        return

    # First child — detach from the process group.
    os.setsid()
    ...
    # Grandchild (daemon) — close inherited fds so nothing ties us to the
    # hook's stdin/stdout/stderr (Claude Code may have pipes here).
